import * as Yup from "yup";
import { useFormik } from "formik";
import { useTranslation } from "react-i18next";
import { useDispatch, useSelector } from "react-redux";
import { setLoading } from "../../../stores/loadingSlice";
import { useEffect, useState } from "react";
import { addVoucherForUser, getUser } from "../../../services/function";
import { fetchVouchers } from "../../../stores/voucherSlice";

export default function ModalAddUserVoucher({ setOpenModal }) {
  const vouchers = useSelector((state) => state.voucher.vouchers);
  const { t } = useTranslation();
  const [user, setUser] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const dispatch = useDispatch();

  useEffect(() => {
    document.title = "Thêm voucher cho thành viên";
    const fetchData = async () => {
      dispatch(setLoading(true));
      try {
        const [res] = await Promise.all([getUser(), dispatch(fetchVouchers())]);
        setUser(res);
        dispatch(setLoading(false));
      } catch (error) {
        dispatch(setLoading(false));
      }
    };
    fetchData();
  }, [dispatch]);

  const filteredUsers = user.filter((u) =>
    u.Email.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const formik = useFormik({
    initialValues: {
      UserID: "",
      VoucherID: "",
    },
    validationSchema: Yup.object({
      UserID: Yup.mixed().required(t("required")),
      VoucherID: Yup.number().required(t("required")),
    }),
    onSubmit: async (values) => {
      console.log(values);
      dispatch(setLoading(true));
      try {
        await addVoucherForUser(values.UserID, values.VoucherID);
        dispatch(setLoading(false));
        setOpenModal(false);
      } catch (error) {
        console.error("Lỗi khi thêm mới voucher:", error);
        dispatch(setLoading(false));
      }
    },
  });

  const userOptions = [
    <option key="all" value="all">
      Tất cả thành viên
    </option>,
    ...filteredUsers.map((u) => (
      <option key={u.UserID} value={u.UserID}>
        {u.Email}
      </option>
    )),
  ];

  const voucherOptions = vouchers.map((v) => (
    <option key={v.VoucherID} value={v.VoucherID}>
      {v.Description}
    </option>
  ));

  return (
    <>
      <div className="fixed inset-8 z-10 overflow-y-auto">
        <div
          className="fixed inset-0 w-full h-full bg-black opacity-40"
          onClick={() => setOpenModal(false)}
        ></div>
        <div className="flex items-center px-4 py-8">
          <div className="relative w-full max-w-lg p-4 mx-auto bg-white rounded-md shadow-lg">
            <div className="mt-3">
              <div className="flex items-center justify-center flex-none w-20 h-20 mx-auto bg-green-500 rounded-full">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
                  fill="currentColor"
                  className="bi bi-database-add w-14 h-14"
                  viewBox="0 0 16 16"
                >
                  <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7m.5-5v1h1a.5.5 0 0 1 0 1h-1v1a.5.5 0 0 1-1 0v-1h-1a.5.5 0 0 1 0-1h1v-1a.5.5 0 0 1 1 0" />
                  <path d="M12.096 6.223A5 5 0 0 0 13 5.698V7c0 .289-.213.654-.753 1.007a4.5 4.5 0 0 1 1.753.25V4c0-1.007-.875-1.755-1.904-2.223C11.022 1.289 9.573 1 8 1s-3.022.289-4.096.777C2.875 2.245 2 2.993 2 4v9c0 1.007.875 1.755 1.904 2.223C4.978 15.71 6.427 16 8 16c.536 0 1.058-.034 1.555-.097a4.5 4.5 0 0 1-.813-.927Q8.378 15 8 15c-1.464 0-2.766-.27-3.682-.687C3.356 13.875 3 13.373 3 13v-1.302c.271.202.58.378.904.525C4.978 12.71 6.427 13 8 13h.027a4.6 4.6 0 0 1 0-1H8c-1.464 0-2.766-.27-3.682-.687C3.356 10.875 3 10.373 3 10V8.698c.271.202.58.378.904.525C4.978 9.71 6.427 10 8 10q.393 0 .774-.024a4.5 4.5 0 0 1 1.102-1.132C9.298 8.944 8.666 9 8 9c-1.464 0-2.766-.27-3.682-.687C3.356 7.875 3 7.373 3 7V5.698c.271.202.58.378.904.525C4.978 6.711 6.427 7 8 7s3.022-.289 4.096-.777M3 4c0-.374.356-.875 1.318-1.313C5.234 2.271 6.536 2 8 2s2.766.27 3.682.687C12.644 3.125 13 3.627 13 4c0 .374-.356.875-1.318 1.313C10.766 5.729 9.464 6 8 6s-2.766-.27-3.682-.687C3.356 4.875 3 4.373 3 4" />
                </svg>
              </div>
              <div className="mt-2 text-center sm:ml-4 sm:text-left">
                <h4 className="text-lg font-medium text-gray-800 text-center">
                  Thêm Voucher cho thành viên
                </h4>
                <form onSubmit={formik.handleSubmit}>
                  <div className="mt-3 text-sm text-gray-600">
                    <label htmlFor="UserID" className="block text-left">
                      Thành viên
                    </label>
                    <input
                      type="text"
                      placeholder="Tìm kiếm email..."
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      className="w-full p-2 mt-1 border border-gray-300 rounded-md outline-none focus:ring-2 focus:ring-indigo-600"
                    />
                    <select
                      id="UserID"
                      name="UserID"
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                      value={formik.values.UserID}
                      className="w-full p-2 mt-1 border border-gray-300 rounded-md outline-none focus:ring-2 focus:ring-indigo-600"
                    >
                      <option value="">Chọn tài khoản</option>
                      {userOptions}
                    </select>
                    {formik.touched.UserID && formik.errors.UserID && (
                      <div className="text-red-500">{formik.errors.UserID}</div>
                    )}
                  </div>

                  <div className="mt-3 text-sm text-gray-600">
                    <label htmlFor="VoucherID" className="block text-left">
                      Voucher
                    </label>
                    <select
                      id="VoucherID"
                      name="VoucherID"
                      onChange={formik.handleChange}
                      onBlur={formik.handleBlur}
                      value={formik.values.VoucherID}
                      className="w-full p-2 mt-1 border border-gray-300 rounded-md outline-none focus:ring-2 focus:ring-indigo-600"
                    >
                      <option value="">Chọn voucher</option>
                      {voucherOptions}
                    </select>
                    {formik.touched.VoucherID && formik.errors.VoucherID && (
                      <div className="text-red-500">
                        {formik.errors.VoucherID}
                      </div>
                    )}
                  </div>

                  <div className="items-center gap-2 mt-3 sm:flex">
                    <button
                      type="submit"
                      className="w-full mt-2 p-2.5 flex-1 text-white bg-red-600 rounded-md outline-none ring-offset-2 ring-red-600 focus:ring-2"
                    >
                      Xác nhận
                    </button>
                    <button
                      type="button"
                      className="w-full mt-2 p-2.5 flex-1 text-gray-800 rounded-md outline-none border ring-offset-2 ring-indigo-600 focus:ring-2"
                      onClick={() => setOpenModal(false)}
                    >
                      Hủy
                    </button>
                  </div>
                </form>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
